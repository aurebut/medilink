  ...
  async function load() {
    ...
    try {
      ...
      const data = await api.get<Application[]>(
        `/establishment/applications?establishmentId=${primary.id}`,
      );
      setItems(data);
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <>
      <PageHeader
        ...
        description="Candidatures reçues sur les missions de ton établissement."
      />
              ...
              {items.map((a) => {
                ...
                return (
                  <tr key={a.id}>
                    <td>
                      <strong>
                        {a.candidate?.profile?.firstName}{' '}
                        {a.candidate?.profile?.lastName}
                      </strong>
                    ...
                    <td>
                      <Badge tone={tone(a.status) as any}>
                      ...
                      {isFinal ? (
                        ...
                        <>
                          <Button
                            variant="success"
                            disabled={isUpdating}
                            onClick={() => void update(a.id, 'ACCEPTED')}
                          >
                            {isUpdating ? '...' : 'Accepter'}
                          </Button>
                          ...
                          <Button
                            variant="danger"
                            disabled={isUpdating}
                            onClick={() => void update(a.id, 'REJECTED')}
                          >
                            {isUpdating ? '...' : 'Refuser'}
                          </Button>
                        </>
