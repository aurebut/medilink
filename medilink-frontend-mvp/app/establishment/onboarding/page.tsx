...
export default function EstablishmentOnboardingPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try { await api.post<Establishment>('/establishments', form); setMessage('Établissement créé.'); await reload(); }
  ...
  return <>
    ...
    <div className="grid-2">
      <Card><h2>Mes établissements</h2>{establishments.length === 0 ? <p>Aucun établissement.</p> : establishments.map((e) => <p key={e.id}><strong>{e.name}</strong><br /><span className="small">{e.type} · {e.verificationStatus}</span></p>)}</Card>
      <Card><h2>Créer un établissement</h2><form className="form" onSubmit={submit}>
        ...
        <Field label="Type"><Select value={form.type} onChange={(e) => set('type', e.target.value as EstablishmentType)}>{establishmentTypeOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</Select></Field>
        <div className="form-row"><Field label="Ville"><Input value={form.city || ''} onChange={(e) => set('city', e.target.value)} /></Field><Field label="Pays"><Input value={form.country || ''} onChange={(e) => set('country', e.target.value)} /></Field></div>
        <Field label="Adresse"><Input value={form.address || ''} onChange={(e) => set('address', e.target.value)} /></Field>
        <div className="form-row"><Field label="Email"><Input type="email" value={form.email || ''} onChange={(e) => set('email', e.target.value)} /></Field><Field label="Téléphone"><Input value={form.phone || ''} onChange={(e) => set('phone', e.target.value)} /></Field></div>
