...
The user also requested replacing the three homepage process previews with connected, abstract vector diagrams. Keep the illustrations in `content/landing-process-art.ts` and their scoped styles at the end of `public/landing-process.css`. Preserve the process section's surrounding structure, copy, tabs, and mobile behavior. The restoration check explicitly allows these three figures and the appended illustration styles while still checking the original page everywhere else.
