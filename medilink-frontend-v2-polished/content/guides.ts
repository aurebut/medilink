...
export type { Guide } from './guide-types';
...
export const guides: Guide[] = [
  {
    ...
    seoTitle: 'Premier remplacement en médecine générale : checklist',
    shortTitle: 'Premier remplacement', topic: 'Débuter', image: guideImages.preparation,
    ...
    sections: [
      ...
      ], checklist: ['Dates exactes et jours que vous pouvez couvrir.', 'Adresse, trajet et éventuel hébergement.', 'Horaires, rendez-vous, visites et secrétariat.', 'Logiciel, équipement et temps de prise en main.', 'Conditions financières et modalités de règlement.'], links: [{ label: 'Comparer deux remplacements avec une grille commune', url: '/guides/choisir-remplacement-medecine-generale' }] },
      ...
      ], checklist: ['Justificatif de votre situation professionnelle.', 'Licence et autorisation si votre situation les exige.', 'Attestation d’assurance à jour.', 'Contrat signé et formalités vérifiées avec le titulaire.', 'Coordonnées utiles pour le premier jour.'], sources: [ordre, ameli], links: [{ label: 'Relire les points essentiels du contrat de remplacement', url: '/guides/contrat-remplacement-medical-points-a-verifier' }] },
    ...
    related: ['choisir-remplacement-medecine-generale', 'contrat-remplacement-medical-points-a-verifier', 'accueillir-medecin-remplacant-cabinet'],
  ...
  },
  {
    ...
    seoTitle: 'Annonce de remplacement médical : modèle et conseils',
    shortTitle: 'Rédiger une annonce', topic: 'Vie du cabinet', image: guideImages.annonce,
    ...
    sections: [
      ...
      ], links: [{ label: 'Comprendre le calcul et les modalités de rétrocession', url: '/guides/retrocession-remplacement-medecine-generale' }] },
    ...
    related: ['accueillir-medecin-remplacant-cabinet', 'retrocession-remplacement-medecine-generale', 'contrat-remplacement-medical-points-a-verifier'],
...
export function readingMinutes(guide: Guide) {
  const words = [guide.intro, guide.takeaway, ...guide.sections.flatMap(section => [section.title, ...section.paragraphs, ...(section.checklist || []), section.template || '', ...(section.table?.headers || []), ...(section.table?.rows.flat() || [])]), ...(guide.faq?.flatMap(item => [item.question, item.answer]) || [])].join(' ').split(/\s+/).length;
