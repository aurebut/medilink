...
export default function NewMissionPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      alert(`Mission créée : ${mission.title}`);
      router.push('/establishment/missions');
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSaving(false);
  ...
  if (!primary) {
    return (
      <>
        ...
        <Card><p>Crée d’abord ton établissement.</p></Card>
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader title="Créer une mission" description={`Établissement : ${primary.name}`} />
      <Card>
        <form className="form" onSubmit={submit}>
          ...
          <div className="form-row">
            <Field label="Type">
              <Select value={form.missionType} onChange={(e) => set('missionType', e.target.value as MissionType)}>
                {missionTypeOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </Select>
            </Field>
            <Field label="Niveau requis">
              <Select value={form.requiredLevel} onChange={(e) => set('requiredLevel', e.target.value as RequiredLevel)}>
                {requiredLevelOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </Select>
            </Field>
          </div>
          <div className="form-row">
            <Field label="Spécialité"><Input required value={form.specialty || ''} onChange={(e) => set('specialty', e.target.value)} /></Field>
          ...
          <div className="form-row">
            <Field label="Heure début"><Input value={form.startTime || ''} placeholder="08:00" onChange={(e) => set('startTime', e.target.value)} /></Field>
            <Field label="Heure fin"><Input value={form.endTime || ''} placeholder="20:00" onChange={(e) => set('endTime', e.target.value)} /></Field>
          </div>
          <div className="form-row">
            <Field label="Durée heures"><Input type="number" min={1} max={72} value={form.durationHours || ''} onChange={(e) => set('durationHours', e.target.value)} /></Field>
            <Field label="Rémunération"><Input type="number" min={0} value={form.compensationAmount || ''} onChange={(e) => set('compensationAmount', e.target.value)} /></Field>
          </div>
