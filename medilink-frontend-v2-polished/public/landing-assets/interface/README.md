...
The desktop message layout and document dossier have square outer corners in the actual application. Their native captures join the macOS toolbar edge to edge without rounded image clipping. Mobile component corners and captures retain their existing presentation.
...
- Mission: `/app/current-missions`, the real mission timeline. The 1600 px desktop viewport keeps the native two-column timeline, establishment photo and candidate profile in a landscape capture. The 390 px mobile viewport captures the full native `.candidate-current-route` with an 8 px margin: compact title/progress, establishment photo and replacement portrait side by side, next-step context and six complete rows. This compact layout is implemented in the actual mobile application, not through capture-only styles. `previewHeight` equals the full image height; the identities, every step, status, description and date fit inside the phone at once, without scrolling or enlargement. The viewport grows to keep fixed navigation outside the image.
